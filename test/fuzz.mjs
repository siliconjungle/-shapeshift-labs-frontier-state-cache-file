...
const seed = readPositiveInt(args.seed, 0xf17ecace);
const rng = mulberry32(seed);
const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'frontier-state-cache-file-fuzz-'));
...
console.log('frontier state-cache-file fuzz passed cases=' + cases + ' seed=' + seed);

async function runCase(caseId, rng) {
  const storage = createQueryCacheFileStorageAdapter({
    directory: path.join(tempRoot, 'case-' + caseId),
    fsync: false,
    maxLogEntries: 16,
    now: () => caseId
  });
