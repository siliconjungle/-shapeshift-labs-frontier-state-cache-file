...
const iterations = readPositiveInt(args.iterations, 24);
const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'frontier-state-cache-file-bench-'));
...
async function measureSave(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'save-' + round),
      fsync: false
    });
  ...
  return row('file snapshot save', samples);
}
...
async function measureLoad(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'load-' + round),
      fsync: false
    });
  ...
  return row('file snapshot load', samples);
}
...
async function measureAppend(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'append-' + round),
      fsync: false
    });
    for (let i = 0; i < iterations; i++) {
      const start = performance.now();
  ...
  return row('file change-log append', samples);
}
...
async function measureReadLog(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'read-log-' + round),
      fsync: false
    });
    for (let i = 0; i < 128; i++) {
  ...
  return row('file change-log read', samples);
}
...
async function measureCompact(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'compact-' + round),
      fsync: false
    });
    const snapshot = makeSnapshot(1000);
    for (let i = 0; i < iterations; i++) {
  ...
  return row('file snapshot compact', samples);
}
...
async function measureClear(rounds, iterations) {
  ...
  for (let round = 0; round < rounds; round++) {
    const storage = createQueryCacheFileStorageAdapter({
      directory: path.join(tempRoot, 'clear-' + round),
      fsync: false
    });
    const snapshot = makeSnapshot(1000);
    for (let i = 0; i < iterations; i++) {
  ...
  return row('file snapshot clear', samples);
}
